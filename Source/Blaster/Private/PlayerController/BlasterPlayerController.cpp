...
void ABlasterPlayerController::CheckPing(float DeltaTime)
{
	...
	{
		// Only check ping on the local client (we display HUD warnings locally)
		if (IsLocalController())
		{
			// Ensure we have a valid PlayerState before calling GetPing()
...
void ABlasterPlayerController::SetHUDShield(float Shield, float MaxShield)
{
	...
	bool bHUDValid = BlasterHUD &&
		BlasterHUD->CharacterOverlay &&
		BlasterHUD->CharacterOverlay->HealthBar && BlasterHUD->CharacterOverlay->ShieldBar;
...
void ABlasterPlayerController::SetHUDGrenades(int32 Grenades)
{
	...
	bool bHUDValid = BlasterHUD &&
	BlasterHUD->CharacterOverlay &&
	BlasterHUD->CharacterOverlay->HealthBar && BlasterHUD->CharacterOverlay->GrenadesText;
	if (bHUDValid)
	{
	...
	if (BlasterHUD)
	{
		...
		if (BlasterHUD->Announcement && BlasterHUD->Announcement->AnnouncementText && BlasterHUD->Announcement->InfoText)
		{
			...
			if (BlasterGameState && BlasterPlayerState)
			{
				...
				{
					InfoTextString = FString(TEXT("Winners: "));
					for (auto TopPlayer : TopPlayers)
					{
						InfoTextString.Append(FString::Printf(TEXT("%s, "), *TopPlayer->GetPlayerName()));
					}
					InfoTextString.RemoveFromEnd(TEXT(", "));
				}
			}
...
void ABlasterPlayerController::HighPingWarning()
{
	...
	bool bHUDValid = BlasterHUD &&
		BlasterHUD->CharacterOverlay &&
		BlasterHUD->CharacterOverlay->HealthBar && BlasterHUD->CharacterOverlay->HighPingImage && 
