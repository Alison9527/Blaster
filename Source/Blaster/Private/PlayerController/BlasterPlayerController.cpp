...
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
	if (bool bHUDValid = BlasterHUD && BlasterHUD->CharacterOverlay && BlasterHUD->CharacterOverlay->BlueTeamScore)
	{
...
void ABlasterPlayerController::ServerCheckMatchState_Implementation()
{
	if (ABlasterGameMode* TempBlasterGameMode = Cast<ABlasterGameMode>(UGameplayStatics::GetGameMode(this)))
	{
		...
		bShowTeamScores = TempBlasterGameMode->IsTeamsMatch(); 
...
void ABlasterPlayerController::SetHUDHealth(float Health, float MaxHealth)
{
	...
	if (bHUDValid)
	{
		...
		// ✅ 修复：成功初始化后清除脏标记，防止重构导致不必要的轮询加载逻辑
...
void ABlasterPlayerController::SetHUDShield(float Shield, float MaxShield)
{
	...
	if (bHUDValid)
	{
		...
		bInitializeShield = false; // ✅ 修复缓存标记遗留
	}
	else
	{
...
void ABlasterPlayerController::SetHUDScore(float Score)
{
	...
	if (bHUDValid)
	{
		...
		bInitializeScore = false; // ✅ 修复缓存标记遗留
	}
	else
	{
...
void ABlasterPlayerController::SetHUDDefeats(int32 Defeats)
{
	...
	if (bHUDValid)
	{
		...
		bInitializeDefeats = false; // ✅ 修复缓存标记遗留
	}
	else
	{
...
void ABlasterPlayerController::SetHUDWeaponAmmo(int32 Ammo)
{
	...
	if (bHUDValid) 
	{
		...
		bInitializeWeaponAmmo = false; // ✅ 修复缓存标记遗留
	}
	else
	{
...
void ABlasterPlayerController::SetHUDCarriedAmmo(int32 Ammo)
{
	...
	if (bHUDValid) 
	{
		...
		bInitializeCarriedAmmo = false; // ✅ 修复缓存标记遗留
	}
	else
	{
...
void ABlasterPlayerController::SetHUDGrenades(int32 Grenades)
{
	...
	if (bHUDValid)
	{
		...
		bInitializeGrenades = false; // ✅ 修复缓存标记遗留
	}
	else
	{
...
FString ABlasterPlayerController::GetInfoText(const TArray<class ABlasterPlayerState*>& Players) const
{
	ABlasterPlayerState* BlasterPlayerState = GetPlayerState<ABlasterPlayerState>();
